...
public class RenderUtils {
    ...
    public static void drawRing(Matrix4f pose, VertexConsumer vertexConsumer, float distance, float originX, float originY, float originZ, float r, float g, float b, float a) {
        drawRing(pose, vertexConsumer, distance, originX, originY, originZ, 15, (pose1, vertexConsumer1, x1, y1, z1, x2, z2, y2) -> {
            vertexConsumer.vertex(pose, x1, y1, z1).color(r, g, b, a).endVertex();
            vertexConsumer.vertex(pose, x2, y1, z2).color(r, g, b, a).endVertex();
            vertexConsumer.vertex(pose, x2, y2, z2).color(r, g, b, a).endVertex();
            vertexConsumer.vertex(pose, x1, y2, z1).color(r, g, b, a).endVertex();
            // Render Inverse
            vertexConsumer.vertex(pose, x1, y2, z1).color(r, g, b, a).endVertex();
            vertexConsumer.vertex(pose, x2, y2, z2).color(r, g, b, a).endVertex();
            vertexConsumer.vertex(pose, x2, y1, z2).color(r, g, b, a).endVertex();
    ...
    public static void drawRing(Matrix4f pose, VertexConsumer vertexConsumer, float distance, float originX, float originY, float originZ, int segments, RingDrawHandler ringDrawHandler) {
